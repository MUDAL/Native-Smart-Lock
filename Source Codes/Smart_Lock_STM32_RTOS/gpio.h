...
#define GPIO_PIN6_INPUT_FLOATING				GPIO_CRL_CNF6_0
...
#define GPIO_PIN7_INPUT_FLOATING				GPIO_CRL_CNF7_0
...
#define GPIO_PIN12_INPUT_PULLUP_OR_PULLDOWN 	GPIO_CRH_CNF12_1
#define GPIO_PIN12_OUTPUT_MODE_2MHZ				GPIO_CRH_MODE12_1

#define GPIO_PIN13								1<<13
#define GPIO_PIN13_INPUT_PULLUP_OR_PULLDOWN 	GPIO_CRH_CNF13_1
...
extern void GPIO_InputInit(GPIO_TypeDef* GPIOx, 
                           uint8_t portLevel,  
                           uint16_t gpioPins,
                           uint32_t config,  
                           bool pullupEn);
//Initializes the GPIO to the output mode of choice
extern void GPIO_OutputInit(GPIO_TypeDef* GPIOx,
                            uint8_t portLevel,
                            uint32_t mode,
                            uint32_t config);
//Writes a logic 'high' or 'low' to a GPIO pin
extern void GPIO_OutputWrite(GPIO_TypeDef* GPIOx,
                             uint16_t gpioPins,
                             bool gpioPinLogic);
//Returns the logic level (high(true) or low(false)) of a GPIO input pin
extern bool GPIO_InputRead(GPIO_TypeDef* GPIOx, uint16_t gpioPin);
//Returns the logic level (high(true) or low(false)) of a GPIO output pin
extern bool GPIO_OutputRead(GPIO_TypeDef* GPIOx, uint16_t gpioPin);
