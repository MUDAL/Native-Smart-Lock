...
{
    USART_WriteByte(USART1,(packet.start_code >> 8));
    USART_WriteByte(USART1,(packet.start_code & 0xFF));
    USART_WriteBytes(USART1,packet.address,4);
    USART_WriteByte(USART1,packet.type);
    
    uint16_t wireLength = packet.length + 2;
    USART_WriteByte(USART1,wireLength >> 8);
    USART_WriteByte(USART1,wireLength & 0xFF);
    
  uint16_t sum = (wireLength >> 8) + (wireLength & 0xFF) + packet.type;
  for (uint8_t i = 0; i < packet.length; i++)
    {
        USART_WriteByte(USART1,packet.data[i]);
    sum += packet.data[i];
  }	
  USART_WriteByte(USART1,sum >> 8);
  USART_WriteByte(USART1,sum & 0xFF);	
}
...
{
    vTaskDelay(pdMS_TO_TICKS(1000)); //Boot-up time
